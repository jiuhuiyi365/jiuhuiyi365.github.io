...
<p>旧版 GUI 每收到一行日志就立即更新界面控件，日志量大时频繁触发 Tkinter 的文本插入和滚动操作，导致 UI 线程阻塞、内存持续增长。</p>

<p>新版做了两处优化：</p>

<ul>
<li><strong>100ms 定时批量刷新：</strong>日志行先写入 <code>deque</code> 缓冲区，每 100ms 统一将缓冲区内容一次写入界面控件，避免每行都触发 UI 刷新</li>
<li><strong>GUI 只保留最后 5000 行：</strong>Log 面板和 Function Calls 面板各最多显示 5000 行，超出部分自动从顶部裁剪，防止界面控件累积大量文本耗尽内存</li>
</ul>

<p>日志文件不受行数限制，始终写入完整的全量数据。</p>
